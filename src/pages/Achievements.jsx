...
import { Eye, Award, Trophy, Medal, Download } from "lucide-react";
...
const CATEGORIES = ["All", "Academic", "Scholarship", "Competition"];
      ...
      <Reveal>
        <div className="mb-6">
          ...
          <p className="font-body text-[18px] text-[#A1A1A1] max-w-xl">
            Awards, scholarships, competitions, and academic recognition.
          </p>
        </div>
      </Reveal>
      ...
      {filtered.length === 0 ? (
        <Reveal>
          <div className="text-center py-20 border border-[#262626] rounded-3xl bg-[#111111]/30">
            <p className="font-mono text-[#A1A1A1] text-lg">No achievements found in this category.</p>
          </div>
        </Reveal>
      ) : (
            ...
            return (
              <Reveal key={a.id} delay={i * 0.06}>
                <div className="group border border-[#262626] hover:border-[#D4AF37]/30 rounded-2xl overflow-hidden bg-[#111111]/30 gold-glow transition-all duration-300 flex flex-col h-full">
                  {/* Preview Area — fixed height for consistent rendering */}
                  ...
                  <div className="p-6 flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      <p className="font-body text-[15px] text-[#A1A1A1] mb-4">{a.org}</p>
                      {a.description && (
                        <p className="font-body text-[14px] leading-relaxed text-[#888] mb-4">{a.description}</p>
                      )}
                    </div>
